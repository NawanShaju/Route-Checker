from core.backend_scanner.RouteScannerStrategy import RouteScannerStrategy, Route
...
class ExpressScanner(RouteScannerStrategy):
    ...
    def scan(self, root=None):
        ...
        server_name, server_file, text = self.__find_server(root)
        paths = self.__find_paths(server_name, server_file, text)
    ...
    def __find_paths(self, server_name, server_file, text) -> list[Route]:
    ...
    def __find_path_in_router(self, routes_matchs, current_file, file_text) -> list[Route]:
        paths: list[Route] = []
                ...
                try:
                    ...
                    if matchs := re.findall(route_regix, text):
                        for m in matchs:
                            ...
                            route = Route(method=method, path=path, file=route_file.name)
                            paths.append(route)
    ...
    def __find_path_in_server(self, paths, server_name, server_text, server_path) -> list[Route]: 
        ...
        if match := re.findall(route_in_server_regix, server_text):
            for m in match:
                ...
                route = Route(method=method, path=path, file=str(server_path))
                paths.append(route)
